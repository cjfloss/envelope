...
namespace Envelope.Service {
    ...
    public class AccountManager : Object {
         ...
         * @throws ServiceError
         */
        public Account create_account (string number, string? description, double balance, Account.Type account_type) throws AccountError, ServiceError {
            ...
            try {
                ...
                if (err is DatabaseError.CONSTRAINT) {
        ...
        public void update_account_balance (ref Account account) throws ServiceError {
            try {
                ...
                dbm.update_account_balance (account);
        ...
        public void update_transaction (Transaction transaction) throws ServiceError {
            ...
            try {
                ...
                // update transaction
                dbm.update_transaction (transaction);
                ...
                dbm.update_account_balance (transaction.account);
        ...
        throws ServiceError {
            try {
                ...
                dbm.delete_transaction (transaction.@id);
        ...
        public int import_transactions_from_file (ref Account account, File file) throws ImporterError, ServiceError {
            ...
            try {
                ...
                if (transactions != null && !transactions.is_empty) {
                    ...
                    info ("adjusting balance for account: %s (new balance: %s)".printf (Envelope.Util.String.format_currency (account.balance), Envelope.Util.String.format_currency (account.balance + balance_delta)));

                    account.balance += balance_delta;

                    dbm.start_transaction ();

                    dbm.insert_transactions (transactions);
                    dbm.update_account_balance (account);
